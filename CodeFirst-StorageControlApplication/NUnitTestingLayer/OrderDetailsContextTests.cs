...
namespace NUnitTestingLayer
{
    public class OrderDetailsContextTests
    {
        ...
        public async Task CreateAsync_ShouldAddOrderDetailToDatabase()
        {
            ...
            var order = new Order(DateTime.Now, new Client("TestClient", "TestAddress", "testemail@test.com", "+123-654-321-098"));
            _dbContext.Orders.Add(order);
            await _dbContext.SaveChangesAsync();

            var orderDetail = new OrderDetail(1, shoe, order);
        ...
        public async Task ReadAllAsync_ShouldReturnAllOrderDetailsFromDatabase()
        {
            ...
            var order1 = new Order(DateTime.Now, new Client("TestClient", "TestAddress", "testemail@test.com", "+123-654-321-098"));
            var order2 = new Order(DateTime.Now, new Client("TestClient2", "TestAddress2", "testemail2@test.com", "+321-654-321-098"));
            _dbContext.Orders.AddRange(order1, order2);
            await _dbContext.SaveChangesAsync();

            var orderDetail1 = new OrderDetail(1, shoe1, order1);
            var orderDetail2 = new OrderDetail(2, shoe2, order2);
